...
-- Songs where this script should be disabled
local excludedSongs = {
--	"takeaway", 
	 "mane"  
	-- "song3",
...
-- Check if current song is excluded
local function isExcludedSong()
	local currentSong = songName or ""
	for i, excludedSong in ipairs(excludedSongs) do
		if currentSong:lower() == excludedSong:lower() then
			return true
		end
	end
	return false
...
-- Global variable to track if script should be active
...
function onCreatePost()
	scriptActive = not isExcludedSong()
